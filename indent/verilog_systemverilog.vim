...
function GetVerilog_SystemVerilogIndent()
  ...
  let match_result = matchstr(curr_line2,'^\<\(end\|else\|end\(case\|task\|function\|clocking\|interface\|module\|class\|specify\|package\|sequence\|group\|property\)\|join\|join_any\|join_none\)\>\|^}\|`endif\|`else\|`ifndef\|`ifdef')
    ...
    if len(match_result) > 0
      ...
      elseif match_result =~ '\<`else\>'
        ...
        if last_line =~ '^\s*`endif\|^\s*}'
          return indent(lnum)
        else
          let match_start = '\<`ifdef\>'
          let match_mid   = ''
          let match_end   = '\<`else\>'
        endif
